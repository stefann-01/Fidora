import { TweetEvidenceData } from '@/app/(app)/mocks/tweet-evidence-mock'
...
export function TweetCard({ tweetData }: TweetCardProps) {
  ...
  const averageSupporting = supportingEvidence.length > 0 
    ? Math.round(supportingEvidence.reduce((sum, item) => sum + item.aiMetric, 0) / supportingEvidence.length)
    : 0
  
  const averageContradicting = contradictingEvidence.length > 0
    ? Math.round(contradictingEvidence.reduce((sum, item) => sum + item.aiMetric, 0) / contradictingEvidence.length)
    : 0
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1">
            {tweetData.evidence
              .sort((a, b) => b.aiMetric - a.aiMetric)
              .slice(0, 2)
              .map((evidence, index) => (
                <div key={index} className="flex items-center justify-between text-xs">
                  ...
                  <Badge 
                    variant="outline" 
                    className={`text-xs ${getMetricColor(evidence.aiMetric)}`}
                  >
                    {evidence.aiMetric}%
                  </Badge>
                </div>
